...
from src.data.stylometrics import extract_stylometric_features, normalize_text
...
def get_dutch_stopwords_lemmatized() -> List[str]:
    ...
    if _dutch_stopwords_lemmatized is None:
        ...
        _dutch_stopwords_lemmatized = list(set([
            token.lemma_.lower() for doc in nlp_model.pipe(raw_stops, batch_size=1000) for token in doc
        ]))
...
class TFIDFParamBuilder:
    SEARCH_SPACES = {
        ...
        "char_min_df": "1 to 4",
        "use_stylometrics": "{True, False}",
        "sty_weight": "[0.01, 10.0] (Log-uniform)",
        "C": "[10^-2, 10^2] (Log-uniform)",
        "linear_loss": "{squared_hinge, hinge}",
        "class_weight": "{balanced, custom [1.0, 5.0]}"
    }
    ...
    def sample_tfidf(trial: Trial, prefix: str, granularity: str = 'full') -> Dict[str, Any]:
        ...
        min_n = trial.suggest_int(f"{prefix}_min_ngram", min_n_low, min_n_high)

        max_n_limit = 2 if (prefix == 'word' and granularity == 'sentence') else (3 if prefix == 'word' else 5)
        max_n = trial.suggest_int(f"{prefix}_max_ngram", min_n, max(min_n, max_n_limit))
        max_feat_low, max_feat_high = (10000, 60000) if granularity == 'sentence' else (20000, 80000)
    ...
    def fit(
        ...
        **kwargs
        ...
        if tune:
            ...
            best_params, tune_sz = SVMOptunaTuner.run(
                train_df=df_train,
                scope=self.scope,
                n_trials=n_trials,
        ...
        try:
            ...
            for trans_name, trans_pipe in feat_union.transformer_list:
                if trans_name == 'word_ngrams':
                    ...
                    sty_names = ["mean_word_len", "var_word_len", "ttr", "hapax", "transitions", "space_ratio", "double_space", "punc_ratio"]
                    if self.scope == 'full':
