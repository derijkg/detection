...
# ==========================================
def load_svm_model_and_config(scope: str, outputs_dir: Path):
    scope_dir = outputs_dir / "svm" / scope
    model_path = scope_dir / "model.joblib"
    config_path = scope_dir / "best_hyperparameters.json"

    if not model_path.exists():
        raise FileNotFoundError(
            f"No trained SVM model found at: '{model_path}'.\n"
            f"Please run 'python scripts/train_svm.py --scopes {scope}' first."
        )
    if not config_path.exists():
        raise FileNotFoundError(f"No configuration JSON found at: '{config_path}'.")

    model = joblib.load(model_path)
    with open(config_path, "r") as f:
        config = json.load(f)

    print(f"[LOADED MODEL] Loaded calibrated Linear SVM from: {model_path}")
    print(f"[LOADED CONFIG] Loaded configuration from: {config_path}")

    return model, config, scope_dir


# ==========================================
# 2. Error Categorization & Analysis Function
# ==========================================
def analyze_and_save_errors(df_eval: pd.DataFrame, scope_dir: Path, scope: str):
    ...
    # 6. Print Top High-Confidence Misclassifications for Debugging
    if len(fp_df) > 0:
...
# ==========================================
# 3. Main Test Evaluation & Plotting Function
# ==========================================
def evaluate_svm_test_set(scope: str, args, manager: DetectionDataManager):
    outputs_base = Path(args.outputs_dir) if args.outputs_dir else DEFAULT_OUTPUTS_DIR
    model, config, scope_dir = load_svm_model_and_config(scope=scope, outputs_dir=outputs_base)

    # Resolve features path dynamically per scope
    if args.features_dir:
        custom_feat = Path(args.features_dir)
        features_dir = custom_feat / f"svm_{scope}" if (custom_feat / f"svm_{scope}").exists() else custom_feat
    else:
        features_dir = DEFAULT_FEATURES_DIR / f"svm_{scope}"

    test_features_path = features_dir / "test.joblib"

    if not test_features_path.exists():
        raise FileNotFoundError(
            f"Pre-extracted test features for scope '{scope}' not found at: {test_features_path}.\n"
            f"Please run 'python scripts/features_svm.py' first."
        )

    # Load test feature matrix and raw DataFrame
    test_data = joblib.load(test_features_path)
    ...
    print("\n" + "=" * 70)
    print(f" EVALUATING SVM MODEL ON HELD-OUT TEST SET [{scope.upper()} SCOPE] ")
    print(f" Optimal Threshold (\u03c4*): {optimal_threshold:.6f}")
    print(f" Test Set Size           : {len(labels)} samples")
    print("=" * 70)

    # 1. Inference (Continuous Probability & Binary Prediction)
    probs_llm = model.predict_proba(X_test)[:, 1]
    ...
    # 3. Compute Per-LLM Generator Breakdown
    ...
    # 4. Perform Detailed Error Analysis and Export CSVs
    ...
    # 5. Generate 4-Panel Figure (300 DPI)
    ...
    with open(latex_table_path, "w") as f:
        f.write("% Auto-generated LaTeX table for test evaluation\n")
        f.write("\\begin{table}[htbp]\n\\centering\n")
        f.write(f"\\caption{{Performance Evaluation of Linear SVM Detector ({scope.upper()}) on Held-out Test Set. Threshold $\\tau^* = {optimal_threshold:.4f}$.}}\n")
        f.write("\\label{tab:svm_" + scope + "_test_results}\n")
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description="Evaluate fine-tuned Linear SVM on held-out test split.")
    ...
    parser.add_argument(
        ...
        help="List of scopes to evaluate sequentially (default: both sentence and full)."
    )
    parser.add_argument("--features_dir", type=str, default=None, help="Custom base features directory path.")
    ...
    for scope in args.scopes:
        evaluate_svm_test_set(scope=scope, args=args, manager=manager)

    print("=" * 70)
    print("[ALL DONE] SVM test evaluation complete for both sentence and full scopes!")
    print("=" * 70)
