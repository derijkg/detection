...
def process_svm_features_for_scope(
    ...
    # 1. Load splits using DetectionDataManager and batch preprocess
    split_data = {}
    for split in splits:
        df_split = manager.filter_dataframe(DataFilter(splits=[split], scopes=[scope]))
        if df_split.empty:
            raise ValueError(f"No records found in dataloader for scope='{scope}' and split='{split}'.")
    ...
    pipeline = get_feature_extraction_pipeline(
        word_tfidf_params=config["word"],
        char_tfidf_params=config["char"],
        granularity=scope
    )

    # 3. Fit pipeline strictly on 'train' split
    if "train" not in split_data:
        raise ValueError("The 'train' split must be present to fit the SVM pipeline.")

    print(f"\n[Fitting Pipeline] Fitting TF-IDF & Stylometrics on 'train' split...")
    pipeline.fit(split_data['train']['records'])

    # 4. Transform and save features
    for split in splits:
        print(f"[Transforming] Generating features for split '{split}'...")
        X_feat = pipeline.transform(split_data[split]['records'])
        ...
        print(f" -> Saved {split} matrix shape {X_feat.shape} to: {save_path}")

    # 5. Save fitted pipeline
    pipeline_save_path = features_dir / "pipeline.joblib"
    joblib.dump(pipeline, pipeline_save_path)
    print(f" -> Saved fitted pipeline to: {pipeline_save_path}")
